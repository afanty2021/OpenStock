...
async function testUSStock(dataPipeline) {
...
async function testAStock(dataPipeline) {
...
async function testHKStock(dataPipeline) {
...
async function testSearch(dataPipeline) {
  section('测试 4: 股票搜索功能');
...
async function testBatchQuotes(dataPipeline) {
  section('测试 5: 批量获取报价');
...
async function testCacheHitRate(dataPipeline) {
  section('测试 6: 缓存命中率');
...
async function main() {
  ...
  try {
    // 动态导入 dataPipeline
    const { dataPipeline } = await import('./lib/data-sources/pipeline.ts');

    await testUSStock(dataPipeline);
    await new Promise(resolve => setTimeout(resolve, 1000));

    await testAStock(dataPipeline);
    await new Promise(resolve => setTimeout(resolve, 1000));

    await testHKStock(dataPipeline);
    await new Promise(resolve => setTimeout(resolve, 1000));

    await testSearch(dataPipeline);
    await new Promise(resolve => setTimeout(resolve, 1000));

    await testBatchQuotes(dataPipeline);
    await new Promise(resolve => setTimeout(resolve, 1000));

    await testCacheHitRate(dataPipeline);
